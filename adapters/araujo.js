...
const BASE_URL = 'https://www.araujo.com.br/';
...
async function openSearch(page, query) {
  ...
  const urls = [
    `https://www.araujo.com.br/search?q=${encoded}`,
    `https://www.araujo.com.br/busca?termo=${encoded}`,
    `https://www.araujo.com.br/${encoded}`
  ];
...
async function scrape({ page, cep, query, site, sameProductVariant }) {
  ...
  if (!searched) {
    return {
      success: false,
      message: 'Não foi possível buscar na Araújo.',
      items: []
    };
  }
  ...
  return {
    success: true,
    message: `${items.length} produto(s) na Araújo. CEP aplicado: ${cepApplied ? 'sim' : 'não confirmado'}.`,
    items
  };
}
